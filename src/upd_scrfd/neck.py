...
class ConvModule(nn.Module):
    ...
    def __init__(self,
        ...
        if self.with_norm:
            ...
            # todo: build_norm_layer(norm_cfg, norm_channels)  # type: ignore
    ...
    def forward(self,
        ...
        for layer in self.order:
            if layer == 'conv':
                ...
                x = self.conv(x)
            elif layer == 'norm' and self.norm is not None: #norm and self.with_norm:
                x = self.norm(x)
            elif layer == 'act' and self.with_activation is not None: #activate and self.with_activation:
                x = self.activate(x)
        return x
...
class FPN(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(self.start_level, self.backbone_end_level):
            l_conv = nn.Conv2d(in_channels[i], out_channels, (1, 1))
            fpn_conv = nn.Conv2d(out_channels, out_channels, (3, 3), padding=1)
        ...
        if self.add_extra_convs and extra_levels >= 1:
            for i in range(extra_levels):
                ...
                extra_fpn_conv = nn.Conv2d(in_channels, out_channels, (3, 3), stride=(2, 2), padding=1,)
...
class PAFPN(FPN):
    ...
    def forward(self, inputs: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        for i, layer in enumerate(self.downsample_convs):
            inter_outs[i + 1] = inter_outs[i + 1] + layer(inter_outs[i])
