...
class SCRFDLoss(nn.Module):
    def __init__(self):
        ...
        self.assigner = ATSSModule()
        self.anchor_generator = AnchorGenerator()

    def kd_compute_stride_loss(self, s_pred, t_pred):



    def compute_stride_loss(self, predictions, targets):
        pos_ind = ...
        if pos_ind > 0:
            bb_loss = self.bb_loss(pos_pred, pos_target)

            kp_loss = self.kp_loss(pos_pred, pos_target)

        cls_loss = self.cls_loss(labels, targets)
    def forward(self, s_pred, t_pred, targets):
        hard_loss_dict = self.compute_loss(s_pred, targets)

        t_pred_targets = self.predictions_to_targets(t_pred)
        soft_loss_dict = self.compute_loss(s_pred, t_pred_targets)

        hint_loss = nn.L1Loss()(s_pred, t_pred)

    def forward(self, predictions, targets, img_shape):

        """
        Predictions bbox            score            iou              kps
        stride8:   (N, 80, 80, 4) + (N, 80, 80, 1) + (N, 80, 80, 1) + (N, 80, 80, 10)
        stride16:  (N, 40, 40, 4) + (N, 40, 40, 1) + (N, 40, 40, 1) + (N, 40, 40, 10)
        stride32:  (N, 20, 20, 4) + (N, 20, 20, 1) + (N, 20, 20, 1) + (N, 20, 20, 10)

        Targets:
        (M, 1+1+4+1+10) = 17?
        """
        anchors = self.anchor_generator(feat_sizes, input_size=img_shape)
        targets = self.prepare_targets(targets)
        targets = self.assigner(targets, anchors)
        ...
        for i, stride in enumerate(self.strides):
            ...
            dt = self.decode_predictions(dt)

            loss = self.compute_stride_loss(dt, gt)
            losses.append(loss)
            num_samples += nw

        loss_dict = agregate_losses(losses, avg_factor=max(1, num_samples))
        return loss_dict
